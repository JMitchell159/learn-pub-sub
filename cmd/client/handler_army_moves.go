...
func handlerWar(gs *gamelogic.GameState, publishCh *amqp.Channel) func(gamelogic.RecognitionOfWar) pubsub.AckType {
	return func(rw gamelogic.RecognitionOfWar) pubsub.AckType {
		defer fmt.Print("> ")
		outcome, win, lose := gs.HandleWar(rw)
		switch outcome {
		...
		case gamelogic.WarOutcomeOpponentWon:
			err := pubsub.PublishGameLog(publishCh, fmt.Sprintf("%s won a war against %s", win, lose), rw.Attacker.Username)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				return pubsub.NackRequeue
			}
		...
		case gamelogic.WarOutcomeDraw:
			err := pubsub.PublishGameLog(publishCh, fmt.Sprintf("A war between %s and %s resulted in a draw", win, lose), rw.Attacker.Username)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				return pubsub.NackRequeue
			}
